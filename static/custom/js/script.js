(function($) {
	...
	if ($('.accordion-box').length) {
		$(".accordion-box").on('click', '.acc-btn', function () {
			...
			if ($(this).hasClass('active')) {
				// If the clicked button is already active, close the accordion content
				$(this).removeClass('active');
				target.removeClass('active-block');
				$(this).next('.acc-content').slideUp(300);
			} else {
				// If the clicked button is not active, open the accordion content
				$(outerBox).find('.accordion .acc-btn').removeClass('active ');
				$(this).addClass('active');
				$(outerBox).children('.accordion').removeClass('active-block');
				$(outerBox).find('.accordion').children('.acc-content').slideUp(300);
	...
	$(document).ready(function() {
		$(".accordion.block").on("click", function() {
			// Remove active class from all accordion blocks
			$(".accordion.block").removeClass("active");
			// Add active class to the clicked accordion block
			$(this).addClass("active");
		});
	});
